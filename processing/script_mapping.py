...
def mapper2(hi_ass,lo_ass,hi_ch,lo_ch,hi_id,lo_id,hi_t,lo_t,pair_id,idmap,inputs,pair_details):
    ...
    for i in range(3):
        ...
        final_uncerts[ind_map[i]] = pos_unc[i] 
    final_coords += np.array(ref_coord)
    hit_layer = inputs["assemblies"][str(hi_ass)]["layer"]
    return final_coords[0],final_coords[1],final_coords[2],hit_t,final_uncerts[0],final_uncerts[1],final_uncerts[2],t_unc,int(hit_layer),int(hi_ass),int(hit_det_id),int(pair_id),hit_ch
...
mapped_tree.Branch("hit_layer",hitlay,"leafname/I")
...
print("Convert raw hit into coordinate")
for x in range(entries):
    ...
    if checkhit(hi_ass,lo_ass,hichannel,lochannel)==1:
        ...
        hitx[0],hity[0],hitz[0],hitt[0],hitxerr[0],hityerr[0],hitzerr[0],hitterr[0],hitlay[0],hitass[0],hitdetid[0],hitpairid[0],hitchid[0]=mapper2(hi_ass,lo_ass,hichannel,lochannel,hi_detid,lo_detid,Tree.time1,Tree.time2,hi_pairid,idmap,inputs,pair_details)
